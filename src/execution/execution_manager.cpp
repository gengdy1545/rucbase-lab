...
void QlManager::insert_into(const std::string &tab_name, std::vector<Value> values, Context *context) {
    ...
    // lab3 task3 Todo end
    auto insertExecutor = std::make_unique<InsertExecutor>(sm_manager_, tab_name, values, context);
    insertExecutor->Next();
}
...
void QlManager::delete_from(const std::string &tab_name, std::vector<Condition> conds, Context *context) {
    ...
    // lab3 task3 Todo end
    auto deleteExecutor = std::make_unique<DeleteExecutor>(sm_manager_, tab_name, conds, rids, context);
    deleteExecutor->Next();
}
...
void QlManager::update_set(const std::string &tab_name, std::vector<SetClause> set_clauses,
    ...
    // make updateExecutor
    auto updateExecutor = std::make_unique<UpdateExecutor>(sm_manager_, tab_name, set_clauses, conds, rids, context);
    updateExecutor->Next();
}
 ...
 * @param orders order by 排序条件
 * @param limit_num limit 选择条数
 */
void QlManager::select_from(std::vector<TabCol> sel_cols, const std::vector<std::string> &tab_names,
    ...
    */
    ...
    /** task2: Order by **/
    std::vector<int> order_col;   // convert string to int
    std::vector<int> order_type;  // convert string to int
    for (auto &order: orders) {
        order_type.push_back(order.order_type.compare("ASC") == 0);
        order_col.push_back(std::find(captions.begin(), captions.end(), order.order_col) - captions.begin());
    }
    ...
    for (executorTreeRoot->beginTuple(); !executorTreeRoot->is_end(); executorTreeRoot->nextTuple()) {
        ...
        records.push_back(columns); // task2: Order by
        // rec_printer.print_record(columns, context);
        num_rec++;
    }
    
    /** task2: Order by **/
    std::sort(records.begin(), records.end(), Record_Cmp(order_col, order_type));
    int output_num = (limit_num == -1) ? records.size() : limit_num;
    for(int i = 0; i < output_num; ++i)
        rec_printer.print_record(records[i], context);
    /********************/
    
    // Print footer
    rec_printer.print_separator(context);
    // Print record count
    RecordPrinter::print_record_count(output_num, context); // change num_rec to output_num
}