    ...
    void interp_sql(const std::shared_ptr<ast::TreeNode> &root, Context *context) {
        if (auto x = std::dynamic_pointer_cast<ast::Help>(root)) {
            ...
            *(context->offset_) = strlen(help_info);
            ...
            for (auto &sv_order_by : x->orders->orderBys) {
                OrderBy sel_order_by = {.order_col = sv_order_by->order_col, .order_type = sv_order_by->order_type};
                orderBys.push_back(sel_order_by);
            }
            ql_manager_->select_from(sel_cols, x->tabs, conds, orderBys, x->orders->limit_num, context);
