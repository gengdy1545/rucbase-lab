...
class DeleteExecutor : public AbstractExecutor {
   ...
   public:
    ...
    std::unique_ptr<RmRecord> Next() override {
        ...
        size_t cols_size = tab_.cols.size();
        for (size_t col_i = 0; col_i < cols_size; col_i++) {
        ...
        for (auto &rid : rids_) {
            ...
            // // add into write_set
            // auto write_record = new WriteRecord(WType::DELETE_TUPLE, tab_name_, *rec);
            ...
            for(size_t i = 0; i < cols_size; ++i) {
                if(tab_.cols[i].index)
                    ihs[i]->delete_entry(rec->data + tab_.cols[i].offset, context_->txn_);
            }
