...
void DiskManager::create_file(const std::string &path) {
    ...
    // 注意不能重复创建相同文件
    if(is_file(path)) { //文件已存在
        throw FileExistsError(path);
    }
    close(open(path.c_str(), O_CREAT, 0644)); //权限rw-r--r--
}
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    if(path2fd_.find(path) == path2fd_.end()) { //文件未打开才删除
        // while(is_file(path))
            unlink(path.c_str());
    }
}
...
int DiskManager::open_file(const std::string &path) {
    ...
    if(path2fd_.find(path) == path2fd_.end()) { //文件未打开才打开
        int fd = open(path.c_str(), O_RDWR);
        if(fd >=0) { //如果成功打开文件,加入文件打开列表
            path2fd_[path] = fd;
            fd2path_[fd] = path;
        }
        return fd;
...
void DiskManager::close_file(int fd) {
    ...
    auto pos = fd2path_.find(fd);
    std::string del_path = pos->second; //提前保存path
    if(pos == fd2path_.end()) { //如果文件未打开
        return;
    }
    close(fd); //关闭文件
    fd2path_.erase(pos); //根据迭代器删除
    path2fd_.erase(del_path); //迭代器已只因，不能通过pos->second获取path了
}
