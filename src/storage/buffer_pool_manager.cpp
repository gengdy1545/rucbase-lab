...
bool BufferPoolManager::FindVictimPage(frame_id_t *frame_id) {
    ...
    if(free_list_.empty()) //缓存池已满需要victim
        return replacer_->Victim(frame_id);
    *frame_id = free_list_.front(); //缓存池未满，直接返回一个可用frame
    free_list_.pop_front();
    return true; //一般来说victim失败才会返回false
}
...
void BufferPoolManager::UpdatePage(Page *page, PageId new_page_id, frame_id_t new_frame_id) {
    ...
    if(page->IsDirty()) { //如果脏页则将脏页写回到page
        disk_manager_->write_page(page->id_.fd, page->id_.page_no, page->data_, PAGE_SIZE);
        page->is_dirty_ = false;
    }
    //更新page table
    page_table_.erase(page->id_); //删除原有PageId
...
Page *BufferPoolManager::FetchPage(PageId page_id) {
    ...
    if(page_table_.find(page_id) != page_table_.end()) { //在缓冲池中
        frame_id_t frame_id = page_table_[page_id]; //在缓冲池中帧编号
        Page* page = &pages_[frame_id]; //获取page
        replacer_->Pin(frame_id); //更新页替换策略信息
        ++page->pin_count_; //增加pin计数
        return page;
    }
    //在disk中需要更新到缓冲池
    frame_id_t frame_id = -1;
    if(FindVictimPage(&frame_id)) { //找到淘汰页
        Page* page = &pages_[frame_id]; 
        //将淘汰页写入disk，清空page内容，并将page_id与缓存池frame_id对应起来
...
bool BufferPoolManager::UnpinPage(PageId page_id, bool is_dirty) {
    ...
    // 2. 页面是否需要置脏
    std::scoped_lock lock{latch_};
    if(page_table_.find(page_id) == page_table_.end()) //不在缓冲池无所谓
        return false;
    frame_id_t frame_id = page_table_[page_id];
    Page* page = &pages_[frame_id];
    if(!page->pin_count_)
        return false; //已经unpin了
    --page->pin_count_;
    if(!page->pin_count_) //如果为0则unpin
        replacer_->Unpin(frame_id);
    page->is_dirty_ = is_dirty; //更新脏页情况
    return true;
}
...
bool BufferPoolManager::FlushPage(PageId page_id) {
    ...
    std::scoped_lock lock{latch_};
    if(page_id.page_no == INVALID_PAGE_ID) //page_id不对
        return false;
    if(page_table_.find(page_id) == page_table_.end()) //缓冲区没这个自然不需要flushdisk
        return false;
    frame_id_t frame_id = page_table_[page_id];
    Page* page = &pages_[frame_id];
...
bool BufferPoolManager::DeletePage(PageId page_id) {
    ...
    frame_id_t frame_id = page_table_[page_id];
    Page* page = &pages_[frame_id];
    if(page->pin_count_) //pinned
        return false;
    disk_manager_->DeallocatePage(page_id.page_no);
    UpdatePage(page, PageId{page_id.fd, INVALID_PAGE_ID}, frame_id);
