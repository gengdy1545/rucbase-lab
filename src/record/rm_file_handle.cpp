...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid &rid, Context *context) const {
    ...
    if(rid.page_no < file_hdr_.num_pages) { //页有效
        RmPageHandle page_handle = fetch_page_handle(rid.page_no);
        if(Bitmap::is_set(page_handle.bitmap, rid.slot_no)) //记录有效
...
Rid RmFileHandle::insert_record(char *buf, Context *context) {
    ...
    Bitmap::set(page_handle.bitmap, slot_no); //设置为已用
    char* free_slot = page_handle.get_slot(slot_no); 
    memcpy(free_slot, buf, file_hdr_.record_size);
    ++page_handle.page_hdr->num_records;
    if(page_handle.page_hdr->num_records == file_hdr_.num_records_per_page)
        file_hdr_.first_free_page_no = page_handle.page_hdr->next_free_page_no; //更新第一个空闲页
...
void RmFileHandle::delete_record(const Rid &rid, Context *context) {
    ...
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    //只需要将bitmap设置为0即可，不需要对slot具体内容进行操作
    Bitmap::reset(page_handle.bitmap, rid.slot_no);
    if(page_handle.page_hdr->num_records == file_hdr_.num_records_per_page)
        release_page_handle(page_handle);
    --page_handle.page_hdr->num_records; //记录减一
    buffer_pool_manager_->UnpinPage(page_handle.page->GetPageId(), true);
}
...
void RmFileHandle::update_record(const Rid &rid, char *buf, Context *context) {
    ...
    // 2. 更新记录
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    char* record = page_handle.get_slot(rid.slot_no);
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    if(page_no >= file_hdr_.num_pages)
        throw PageNotExistError("tableName", page_no);
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    // 3.更新file_hdr_
    Page* new_page = buffer_pool_manager_->NewPage(new PageId{fd_, INVALID_PAGE_ID});
    RmPageHandle page_handle = RmPageHandle(&file_hdr_, new_page); 
    page_handle.page_hdr->next_free_page_no = file_hdr_.first_free_page_no; //加到第一个空闲页之前
    page_handle.page_hdr->num_records = 0; //初始化0
    Bitmap::init(page_handle.bitmap, file_hdr_.bitmap_size);
    ++file_hdr_.num_pages; //增加分配page数量
